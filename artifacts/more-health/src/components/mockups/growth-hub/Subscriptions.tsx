...
export function Subscriptions() {
  ...
  const SUBSCRIPTIONS = [
    { id: "SUB-001", product: "SomaDerm Transdermal Gel", freq: t("Monthly", "月度"), date: "Nov 02, 2023", amount: "¥1,224.00", pm: "Visa •••• 4242", city: t("Shanghai", "上海"), status: "Active", image: "/images/somaderm.png" },
    { id: "SUB-002", product: "Revitalize Eye Cream", freq: t("Monthly", "月度"), date: "Nov 05, 2023", amount: "¥560.00", pm: t("WeChat Pay", "微信支付"), city: t("Beijing", "北京"), status: "Active", image: "/images/revitalize-eye-cream.png" },
    { id: "SUB-003", product: "TRi-M*LT Liquid Shot", freq: t("Bi-Monthly", "双月"), date: "Dec 10, 2023", amount: "¥828.00", pm: t("Alipay", "支付宝"), city: t("Shenzhen", "深圳"), status: "Paused", image: "/images/tri-mlt.png" },
  ];

  const statusLabel = (s: string) => s === "Active" ? t("Active", "进行中") : s === "Paused" ? t("Paused", "已暂停") : s;
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-card p-6 rounded-2xl border border-border/50 shadow-sm">
          <div className="flex gap-8">
            <div className="flex flex-col gap-1">
              <h1 className="text-2xl font-display font-bold tracking-tight">{t("Active Subscriptions", "活跃订阅")}</h1>
              <p className="text-muted-foreground text-sm">{t("Recurring orders from your community.", "来自社群的持续订阅订单。")}</p>
            </div>
            <div className="w-px bg-border/80 hidden sm:block"></div>
            <div className="flex gap-8">
              <div className="flex flex-col">
                <span className="text-xs text-muted-foreground uppercase font-semibold tracking-wider">{t("Count", "数量")}</span>
                <span className="text-2xl font-bold font-display">142</span>
              </div>
              <div className="flex flex-col">
                <span className="text-xs text-muted-foreground uppercase font-semibold tracking-wider">{t("Est. MRR", "预估月收入")}</span>
          ...
          <Button className="gap-2 shadow-sm rounded-xl bg-primary text-primary-foreground h-11 px-6">
            <Plus className="w-4 h-4" /> {t("Add Subscription", "新增订阅")}
          </Button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-48 rounded-xl">
                        <DropdownMenuItem>{t("Edit Products", "编辑产品")}</DropdownMenuItem>
                        <DropdownMenuItem>{t("Change Address", "修改地址")}</DropdownMenuItem>
                        <DropdownMenuItem>{t("Change Payment", "更换支付方式")}</DropdownMenuItem>
                        <DropdownMenuItem>{t("Skip Next", "跳过下一次")}</DropdownMenuItem>
                        {sub.status === "Active" ? (
                          <DropdownMenuItem className="text-amber-600">{t("Pause Subscription", "暂停订阅")}</DropdownMenuItem>
                        ) : (
                          <DropdownMenuItem className="text-emerald-600">{t("Resume Subscription", "恢复订阅")}</DropdownMenuItem>
                        )}
                        <DropdownMenuItem className="text-destructive">{t("Cancel", "取消")}</DropdownMenuItem>
                      </DropdownMenuContent>
                    </DropdownMenu>
                  </div>
              ...
              <CardContent className="p-5 space-y-4">
                ...
                <div className="space-y-3 pt-2">
                  <div className="flex items-center gap-3 text-sm">
                    ...
                    <span className="text-muted-foreground w-24">{t("Next ship:", "下次发货：")}</span>
                    <span className="font-medium">{sub.date}</span>
                  </div>
                  <div className="flex items-center gap-3 text-sm">
                    <CreditCard className="w-4 h-4 text-muted-foreground" />
                    <span className="text-muted-foreground w-24">{t("Payment:", "支付方式：")}</span>
                    <span className="font-medium">{sub.pm}</span>
                  </div>
                  <div className="flex items-center gap-3 text-sm">
                    <MapPin className="w-4 h-4 text-muted-foreground" />
                    <span className="text-muted-foreground w-24">{t("Shipping:", "收货地：")}</span>
          ...
          <Card className="border-2 border-dashed border-border/60 rounded-2xl bg-transparent hover:bg-secondary/30 transition-colors cursor-pointer flex flex-col items-center justify-center min-h-[300px] text-muted-foreground hover:text-foreground group">
            ...
            <p className="font-semibold text-lg">{t("New Subscription", "新增订阅")}</p>
            <p className="text-sm mt-1 text-center px-6">{t("Set up auto-ship for yourself or a customer.", "为自己或客户设置自动续订。")}</p>
          </Card>
        </div>
